...
# The plugin displays tasks from Omnifocus 3, divided into categories, sorted by date. Default categories are Past due, Now (within 2 days), Soon (within a month), and Home. They can be modified. Colors can be assigned to each category. It uses a combination of Python and AppleScript.
# 
...
categoryDefinitions = [
['Past',    1, 3, 'red',    'red',      '\x1b[31m', 'task.duedate < datetime.now()'],
['Now',     2, 4, 'black',  'white',    '\x1b[0m',  'task.duedate > datetime.now() and task.duedate <= (datetime.now() + timedelta(days=2))'],
['Soon',    3, 5, 'black',  'white',    '\x1b[0m',  'task.duedate > (datetime.now() + timedelta(days=2)) and task.duedate <= (datetime.now() + timedelta(days=32))'],
['Home',    4, 2, 'black',  'white',    '\x1b[0m',  '(task.project == "Home" or task.tag == "home") and task.duedate <= (datetime.now() + timedelta(days=32))'],
['Waiting', 5, 1, 'black',  'white',    '\x1b[0m',  '"Waiting" in task.tag']
]
...
class tasks:
    ...
    def __init__(self, taskID, taskName, dueDate, project, tag):
        self.taskID = taskID
        self.name = taskName
        ...
        on run taskID
        	tell application "OmniFocus"
        		tell default document
        			if (count (every remaining task of every flattened tag)) > 0 then
        				repeat with aTask in (every remaining task of every flattened tag)
    		...
    		if (count (every remaining task of every flattened tag)) > 0 then
    			repeat with aTask in (every remaining task of every flattened tag)
    				if number of tasks of aTask is 0 and due date of aTask is not missing value then
    					...
    					if the name of the tag of aTask is not "XXXX" then	
                        if task_IDs does not contain id of aTask then
                            set end of task_IDs to id of aTask
    						set end of the_list to {\
                                id of aTask, \
                                name of aTask as string, \
                                due date of aTask, \
                                name of containing project of aTask as string, \
                                name of tag of aTask}
    					end if
                        end if
    				end if
    			end repeat
...
def contruct_menubar_text(tasksByCategory):
    ...
    menubarTextDict={}
    for i in range(len(categoryDefinitions)): 
